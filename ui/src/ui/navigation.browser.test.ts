...
const originalConnect = NEMOApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("nemo-app") as NEMOApp;
...
beforeEach(() => {
  NEMOApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__NEMO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  NEMOApp.prototype.connect = originalConnect;
  window.__NEMO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/nemo/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/nemo");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/nemo/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__NEMO_CONTROL_UI_BASE_PATH__ = "/nemo";
    const app = mountApp("/nemo/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/nemo");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/nemo/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("nemo.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
