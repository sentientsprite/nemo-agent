...
struct NEMOMacCLI {
    static func main() async {
        ...
        default:
            fputs("nemo-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      nemo-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      nemo-mac discover [--timeout <ms>] [--json] [--include-local]
      nemo-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      nemo-mac connect
      nemo-mac connect --url ws://127.0.0.1:18789 --json
      nemo-mac discover --timeout 3000 --json
      nemo-mac wizard --mode local
    """)
}
