...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.nemo/nemo.json. Prefer NEMO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.nemo/nemo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.nemo/nemo.json. Restart the gateway to apply."
        }
