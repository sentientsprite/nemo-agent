...
private let webChatSwiftLogger = Logger(subsystem: "ai.nemo", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: NEMOChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NEMOChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(NEMOChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NEMOChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NEMOGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<NEMOChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any NEMOChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = NEMOChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: NEMOChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<NEMOChatView>) -> NSViewController
    {
