...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NEMOProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.nemo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NEMOKit.AnyCodable]()) {
                $0[$1.key] = NEMOKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NEMOProtocol.AnyCodable?) -> [String: NEMOProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NEMOProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NEMOKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NEMOProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("nemo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("nemo.control.agent")
}
