...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.nemo.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["NEMO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "nemo.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "nemo.nixMode") { return true }
