...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: NEMOOAuthStore.AnthropicOAuthStatus = NEMOOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: NEMOOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = NEMOOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = NEMOOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try NEMOOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. NEMO can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: NEMOOAuthStore.AnthropicOAuthStatus,
