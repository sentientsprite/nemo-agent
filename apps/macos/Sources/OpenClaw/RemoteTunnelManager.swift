...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.nemo", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous NEMO run already has an SSH listener on the expected port (common after restarts),
