...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (NEMO token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: NEMOOAuthStore.AnthropicOAuthStatus = NEMOOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.nemo", category: "anthropic-oauth")
...
enum NEMOOAuthStore {
    ...
    private static let nemoOAuthDirEnv = "NEMO_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "NEMO OAuth token file not found"
            case .unreadableFile: "NEMO OAuth token file not readable"
            case .invalidJSON: "NEMO OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in NEMO OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "NEMO OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.nemoOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".nemo", isDirectory: true)
