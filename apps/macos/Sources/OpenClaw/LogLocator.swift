...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NEMO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/nemo")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("nemo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("nemo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/nemo/ (rolling or stdout), or nil if none exist.
