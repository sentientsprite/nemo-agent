...
enum CommandResolver {
    private static let projectRootDefaultsKey = "nemo.gatewayProjectRootPath"
    private static let helperName = "nemo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let nemoEntry = root.appendingPathComponent("nemo.mjs").path
        if FileManager().isReadableFile(atPath: nemoEntry) { return nemoEntry }
        let binEntry = root.appendingPathComponent("bin/nemo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/nemo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let nemoPaths = self.nemoManagedPaths(home: home)
        if !nemoPaths.isEmpty {
            extras.insert(contentsOf: nemoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + nemoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func nemoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".nemo"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nemoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNEMOExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("nemo.mjs").path,
            root.appendingPathComponent("bin/nemo.js").path,
        ]
    ...
    static func hasAnyNEMOInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.nemoExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let nemoPath = self.projectNEMOExecutable(projectRoot: root) {
                return [nemoPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "nemo", subcommand] + extraArgs
            }
            if let nemoPath = self.nemoExecutable(searchPaths: searchPaths) {
                return [nemoPath, subcommand] + extraArgs
            }

            let missingEntry = """
            nemo entrypoint missing (looked for dist/index.js or nemo.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real nemo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/nemo"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/nemo.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/nemo.mjs"
            node "$PRJ/nemo.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/nemo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/nemo.js"
            node "$PRJ/bin/nemo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent nemo"
          pnpm --silent nemo \(quotedArgs);
        else
          echo "nemo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? NEMOConfigFile.loadDict()
