...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.nemo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> NEMOSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: NEMOSessionPreviewEntry,
    ...
    private static func cache(payload: NEMOSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: NEMOChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [NEMOKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [NEMOKit.AnyCodable]) -> [NEMOChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(NEMOChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: NEMOChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: NEMOChatMessage) -> Bool {
    ...
    private static func toolNames(for message: NEMOChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: NEMOChatMessage) -> String? {
