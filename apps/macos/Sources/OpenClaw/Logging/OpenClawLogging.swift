...
enum NEMOLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = NEMOOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = NEMOFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        NEMOLogging.bootstrapIfNeeded()
        let label = NEMOLogging.makeLabel(subsystem: subsystem, category: category)
...
struct NEMOOSLogHandler: LogHandler {
...
struct NEMOFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = NEMOLogging.parseLabel(self.label)
