...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.nemo", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: NEMOPeekabooBridgeServices?
    private static var nemoSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("NEMO", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = NEMOPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.nemoSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.nemoSocketPath, privacy: .public)")
    }
...
@MainActor
private final class NEMOPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.nemo.peekaboo")
