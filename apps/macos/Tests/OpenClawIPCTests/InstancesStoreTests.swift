...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: NEMOProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NEMOProtocol.AnyCodable] = [
            "presence": .init([NEMOProtocol.AnyCodable(entry)]),
        ]
        let payload = NEMOProtocol.AnyCodable(payloadMap)
