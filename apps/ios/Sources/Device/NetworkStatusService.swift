...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> NEMONetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> NEMONetworkStatusPayload {
        let status: NEMONetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> NEMONetworkStatusPayload {
        NEMONetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
