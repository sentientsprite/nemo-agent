...
final class MotionService: MotionServicing {
    func activities(params: NEMOMotionActivityParams) async throws -> NEMOMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NEMOMotionActivityEntry], Error>) in
        ...
        return NEMOMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: NEMOPedometerParams) async throws -> NEMOPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<NEMOPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = NEMOPedometerPayload(
