...
final class ContactsService: ContactsServicing {
    ...
    func search(params: NEMOContactsSearchParams) async throws -> NEMOContactsSearchPayload {
        ...
        return NEMOContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: NEMOContactsAddParams) async throws -> NEMOContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return NEMOContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return NEMOContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> NEMOContactPayload {
        NEMOContactPayload(
