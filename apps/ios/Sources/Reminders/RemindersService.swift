...
final class RemindersService: RemindersServicing {
    func list(params: NEMORemindersListParams) async throws -> NEMORemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[NEMOReminderPayload], Error>) in
        ...
        return NEMORemindersListPayload(reminders: payload)
    }
    ...
    func add(params: NEMORemindersAddParams) async throws -> NEMORemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = NEMOReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return NEMORemindersAddPayload(reminder: payload)
    }
