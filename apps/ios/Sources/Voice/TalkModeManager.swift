...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<NEMOTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = NEMOTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> NEMOTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return NEMOTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> NEMOTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> NEMOTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> NEMOTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = NEMOTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: NEMOTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: NEMOAgentEventPayload.self) else {
                continue
            }
