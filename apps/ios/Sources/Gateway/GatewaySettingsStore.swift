...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.nemo.gateway"
    private static let nodeService = "ai.nemo.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.nemo.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.nemo.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("nemo-gateway.log")
    }
