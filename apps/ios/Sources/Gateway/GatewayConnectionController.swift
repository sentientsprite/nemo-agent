...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [NEMOCapability.canvas.rawValue, NEMOCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(NEMOCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(NEMOCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = NEMOLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(NEMOCapability.location.rawValue) }

        caps.append(NEMOCapability.device.rawValue)
        caps.append(NEMOCapability.photos.rawValue)
        caps.append(NEMOCapability.contacts.rawValue)
        caps.append(NEMOCapability.calendar.rawValue)
        caps.append(NEMOCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(NEMOCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            NEMOSystemCommand.notify.rawValue,
            NEMOChatCommand.push.rawValue,
        ...
        if caps.contains(NEMOCapability.camera.rawValue) {
            commands.append(NEMOCameraCommand.list.rawValue)
            commands.append(NEMOCameraCommand.snap.rawValue)
            commands.append(NEMOCameraCommand.clip.rawValue)
        }
        if caps.contains(NEMOCapability.location.rawValue) {
            commands.append(NEMOLocationCommand.get.rawValue)
        }
        if caps.contains(NEMOCapability.device.rawValue) {
            commands.append(NEMODeviceCommand.status.rawValue)
            commands.append(NEMODeviceCommand.info.rawValue)
        }
        if caps.contains(NEMOCapability.photos.rawValue) {
            commands.append(NEMOPhotosCommand.latest.rawValue)
        }
        if caps.contains(NEMOCapability.contacts.rawValue) {
            commands.append(NEMOContactsCommand.search.rawValue)
            commands.append(NEMOContactsCommand.add.rawValue)
        }
        if caps.contains(NEMOCapability.calendar.rawValue) {
            commands.append(NEMOCalendarCommand.events.rawValue)
            commands.append(NEMOCalendarCommand.add.rawValue)
        }
        if caps.contains(NEMOCapability.reminders.rawValue) {
            commands.append(NEMORemindersCommand.list.rawValue)
            commands.append(NEMORemindersCommand.add.rawValue)
        }
        if caps.contains(NEMOCapability.motion.rawValue) {
            commands.append(NEMOMotionCommand.activity.rawValue)
            commands.append(NEMOMotionCommand.pedometer.rawValue)
        }
