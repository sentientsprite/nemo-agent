...
struct IOSGatewayChatTransport: NEMOChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> NEMOChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(NEMOChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> NEMOChatHistoryPayload {
        ...
        return try JSONDecoder().decode(NEMOChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [NEMOChatAttachmentPayload]) async throws -> NEMOChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [NEMOChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(NEMOChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(NEMOGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NEMOGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: NEMOChatEventPayload.self)
                        {
