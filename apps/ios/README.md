...
This is an **alpha** iOS app that connects to an NEMO Gateway as a `role: node`.
...
3. On iOS: NEMO → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project NEMO.xcodeproj -scheme NEMO -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/NEMOKit` contains the shared transport/types used by the iOS app.
