...
private let chatUILogger = Logger(subsystem: "ai.nemo", category: "NEMOChatUI")

@MainActor
@Observable
public final class NEMOChatViewModel {
    public private(set) var messages: [NEMOChatMessage] = []
    ...
    public var attachments: [NEMOPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NEMOChatPendingToolCall] = []
    public private(set) var sessions: [NEMOChatSessionEntry] = []
    private let transport: any NEMOChatTransport
    ...
    private var pendingToolCallsById: [String: NEMOChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any NEMOChatTransport) {
    ...
    public var sessionChoices: [NEMOChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [NEMOChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [NEMOChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: NEMOPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [NEMOChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NEMOChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [NEMOChatMessage]) -> [NEMOChatMessage] {
        var result: [NEMOChatMessage] = []
    ...
    private static func dedupeKey(for message: NEMOChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [NEMOChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> NEMOChatAttachmentPayload in
            NEMOChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> NEMOChatSessionEntry {
        NEMOChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: NEMOChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: NEMOChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: NEMOAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NEMOChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> NEMOPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
