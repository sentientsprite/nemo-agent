...
@MainActor
struct NEMOChatComposer: View {
    @Bindable var viewModel: NEMOChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(NEMOChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(NEMOChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NEMOPendingAttachment.id)
                { (att: NEMOPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NEMOPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(NEMOChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(NEMOChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(NEMOChatTheme.subtleCard)
        .clipShape(Capsule())
    }
