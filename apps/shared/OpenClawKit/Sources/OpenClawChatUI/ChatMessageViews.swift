...
struct ChatMessageBubble: View {
    let message: NEMOChatMessage
    let style: NEMOChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? NEMOChatTheme.userText : NEMOChatTheme.assistantText
    ...
    private var inlineAttachments: [NEMOChatMessageContent] {
    ...
    private var toolCalls: [NEMOChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [NEMOChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? NEMOChatTheme.userBubble
        }
        if self.style == .onboarding {
            return NEMOChatTheme.onboardingAssistantBubble
        }
        return NEMOChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return NEMOChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: NEMOChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? NEMOChatTheme.userText : NEMOChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: NEMOChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(NEMOChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [NEMOChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: NEMOChatTheme.assistantText)
            }
        }
    }
}
