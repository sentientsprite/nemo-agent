...
private final class TestChatTransport: @unchecked Sendable, NEMOChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NEMOChatHistoryPayload]
    private let sessionsResponses: [NEMOChatSessionsListResponse]

    private let stream: AsyncStream<NEMOChatTransportEvent>
    private let continuation: AsyncStream<NEMOChatTransportEvent>.Continuation

    init(
        historyResponses: [NEMOChatHistoryPayload],
        sessionsResponses: [NEMOChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<NEMOChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> NEMOChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NEMOChatHistoryPayload(
        ...
        attachments _: [NEMOChatAttachmentPayload]) async throws -> NEMOChatSendResponse
    {
        ...
        return NEMOChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> NEMOChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? NEMOChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: NEMOChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = NEMOChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = NEMOChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { NEMOChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = NEMOChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = NEMOChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = NEMOChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { NEMOChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
