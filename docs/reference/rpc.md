...
NEMO integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- NEMO owns lifecycle when `channels.signal.autoStart=true`.
...
- NEMO spawns `imsg rpc` as a child process (legacy iMessage integration).
