...
NEMO features that can generate provider usage or paid API calls.
...
- `nemo status --usage` and `nemo channels list` show provider **usage windows**
...
NEMO can pick up credentials from:
...
`nemo models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
