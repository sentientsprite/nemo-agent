...
NEMO supports **camera capture** for agent workflows:
...
```bash
nemo nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
nemo nodes camera snap --node <id> --facing front
nemo nodes camera clip --node <id> --duration 3000
nemo nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`nemo.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `nemo` CLI to invoke camera commands on the macOS node.

Examples:

```bash
nemo nodes camera list --node <id>            # list camera ids
nemo nodes camera snap --node <id>            # prints MEDIA:<path>
nemo nodes camera snap --node <id> --max-width 1280
nemo nodes camera snap --node <id> --delay-ms 2000
nemo nodes camera snap --node <id> --device-id <id>
nemo nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
nemo nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
nemo nodes camera clip --node <id> --device-id <id>
nemo nodes camera clip --node <id> --no-audio
```

Notes:

- `nemo nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
nemo nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
