...
- Same path: run the app from a fixed location (for NEMO, `dist/NEMO.app`).
...
Workaround: move files into the NEMO workspace (`~/.nemo/workspace`) if you want to avoid per-folder grants.
