...
service via the external `nemo` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`NEMO_PROFILE`; legacy `com.nemo.*` is supported).
