---
summary: "NEMO on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up NEMO on a Raspberry Pi
  - Running NEMO on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# NEMO on Raspberry Pi

## Goal

Run a persistent, always-on NEMO Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most NEMO features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled nemo

# Enable if not
sudo systemctl enable nemo

# Start on boot
sudo systemctl start nemo
```
...
```bash
# Check logs
journalctl -u nemo --no-pager -n 100

# Common fix: rebuild
cd ~/nemo  # if using hackable install
npm run build
sudo systemctl restart nemo
```
