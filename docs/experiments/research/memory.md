---
summary: "Research notes: offline memory system for NEMOd workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.nemo/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep NEMO integration
...
Target: NEMOd-style workspace (`agents.defaults.workspace`, default `~/.nemo/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.nemo/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want NEMOd to see every session.
...
```
~/.nemo/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in NEMO**, but keep a separable core library.

### Why integrate into NEMO?
...
- You want the agent itself to call the tools:
  - `nemo memory recall "…" --k 25 --since 30d`
  - `nemo memory reflect --since 7d`
...
Pragmatic take for `~/.nemo/workspace`:
