...
- Jobs persist under `~/.nemo/cron/` so restarts don’t lose schedules.
...
nemo cron list
nemo cron run <job-id>
nemo cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.nemo/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `nemo cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, NEMO defaults to `announce`.
...
- Job store: `~/.nemo/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.nemo/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
nemo cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
nemo cron edit <jobId> --agent ops
nemo cron edit <jobId> --clear-agent
```
...
```bash
nemo cron run <jobId>
nemo cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
nemo cron edit <jobId> \
...
```bash
nemo system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`nemo system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `NEMO_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- NEMO applies exponential retry backoff for recurring jobs after consecutive errors:
