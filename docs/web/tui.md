...
```bash
nemo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `nemo tui --deliver`
...
- Check the Gateway logs: `nemo logs --follow`.
- Confirm the agent can run: `nemo status` and `nemo models status`.
...
- No agents in picker: check `nemo agents list` and your routing config.
