...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/nemo`)
...
If the page fails to load, start the Gateway first: `nemo gateway`.
...
you revoke it with `nemo devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.nemo/nemo.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. NEMO
...
NEMO **blocks** Control UI connections without device identity.
