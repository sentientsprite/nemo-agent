...
read_when:
  - You need detailed behavior for nemo onboard
...
This page is the full reference for `nemo onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nemo/nemo.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `nemo doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `nemo pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `nemo health`.
    - `nemo status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.nemo/.env` so launchd can read it.
...
- OAuth credentials: `~/.nemo/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.nemo/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.nemo/credentials/oauth.json` (or `$NEMO_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`nemo onboard`](/cli/onboard)
