...
Deploy NEMO on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy NEMO](https://northflank.com/stacks/deploy-nemo) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy NEMO now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the NEMO template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the NEMO service.
8. Open the public NEMO URL and complete setup at `/setup`.
9. Open the Control UI at `/nemo`.
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/nemo`
