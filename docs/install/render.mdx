...
Deploy NEMO on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/nemo/nemo)
...
repository configures everything needed to run NEMO:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: NEMO_STATE_DIR
        value: /data/.nemo
      - key: NEMO_WORKSPACE_DIR
        value: /data/workspace
      - key: NEMO_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/nemo`.
...
If you use the original NEMO repository, Render will not auto-deploy your NEMO. To update it, run a manual Blueprint sync from the dashboard.
...
For NEMO, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any NEMO host.
