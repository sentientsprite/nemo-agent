...
Deploy NEMO on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/nemo-agent-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/nemo` — Control UI

## What you get

- Hosted NEMO Gateway + Control UI
...
- `NEMO_STATE_DIR=/data/.nemo` (recommended)
- `NEMO_WORKSPACE_DIR=/data/workspace` (recommended)
- `NEMO_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your NEMO state + workspace so you can migrate to another host without losing config or memory.
