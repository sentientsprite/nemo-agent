...
- **gateway host** → `nemo` process on the gateway machine
...
`~/.nemo/exec-approvals.json` directly.

CLI: `nemo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
