---
summary: "Direct `nemo agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `nemo agent` (direct agent runs)

`nemo agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `nemo message --target`).
...
```bash
nemo agent --to +15555550123 --message "status update"
...
nemo agent --to +15555550123 --message "Summon reply" --deliver
