---
summary: "Chrome extension: let NEMO drive your existing Chrome tab"
read_when:
...
The NEMO Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate nemo-managed Chrome profile.
...
NEMO then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the NEMO release (npm package) as static files. There is no separate “build” step.

After upgrading NEMO:

- Re-run `nemo browser extension install` to refresh the installed files under your NEMO state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

NEMO ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want NEMO to control.
...
- `ON`: attached; NEMO can drive that tab.
...
Debugging: `nemo sandbox explain`
...
`nemo browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `nemo browser extension install` first to copy the extension to a stable location under your NEMO state directory.
...
- **This is not isolated** like the dedicated nemo-managed profile.
