---
summary: "Agent tool surface for NEMO (browser, canvas, nodes, message, cron) replacing legacy `nemo-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `nemo-*` skills
title: "Tools"
---

# Tools (NEMO)

NEMO exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `nemo-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `nemo.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, NEMO logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:nemo`: all built-in NEMO tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`nemo node run`).
...
- Requires a Brave API key (recommended: `nemo configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated NEMO-managed browser.
...
- Quick smoke: `nemo nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `nemo gateway` restart in-place)
