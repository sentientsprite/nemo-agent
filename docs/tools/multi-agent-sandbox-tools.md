...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `nemo sandbox explain`.
...
Legacy `agent.*` configs are migrated by `nemo doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=nemo-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${NEMO_STATE_DIR:-$HOME/.nemo}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
