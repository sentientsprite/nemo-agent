...
- During heartbeat runs, NEMO treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `nemo sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, NEMO skips the heartbeat run entirely (no model call).
...
```bash
nemo system event --text "Check for urgent follow-ups" --mode now
```
