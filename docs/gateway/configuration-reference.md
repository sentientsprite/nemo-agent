...
description: "Complete field-by-field reference for ~/.nemo/nemo.json"
---

# Configuration Reference

Every field available in `~/.nemo/nemo.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — NEMO uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `nemo doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `nemo plugins install @nemo/mattermost`.
...
NEMO spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@nemo"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `nemo.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, NEMO auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, NEMO assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.nemo/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.nemo/workspace",
        agentDir: "~/.nemo/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@nemo"] },
        sandbox: { mode: "off" },
...
NEMO uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.nemo/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `NEMO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `nemo onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `nemo onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `nemo onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `nemo onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `nemo onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `nemo onboard --auth-choice minimax-api`.
...
- Loaded from `~/.nemo/extensions`, `<workspace>/.nemo/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or NEMO_GATEWAY_PASSWORD
      allowTailscale: true,
    },
    tailscale: {
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `NEMO_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.nemo-dev` + port `19001`), `--profile <name>` (uses `~/.nemo-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-nemo-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `NEMO_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or NEMO_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__nemo__/a2ui/`.
...
- Hostname defaults to `nemo`. Override with `NEMO_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.nemo/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `nemo dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.nemo/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.nemo/credentials/oauth.json`.
...
- Default log file: `/tmp/nemo/nemo-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `nemo doctor --fix` can strip unknown keys).
