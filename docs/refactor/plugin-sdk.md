...
- Publish as `nemo/plugin-sdk` (or export from core under `nemo/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `NEMOPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: NEMOConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: NEMOConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: NEMOConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: NEMOConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: NEMOConfig): string;
  };
};
```
...
- Introduce `nemo/plugin-sdk`.
- Add `api.runtime` to `NEMOPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `nemoRuntime: ">=2026.2.0"`).
