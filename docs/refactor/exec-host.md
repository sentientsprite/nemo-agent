...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.nemo/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.nemo/node.json` (node id + pairing token).
...
- Unix socket at `~/.nemo/exec-approvals.sock` (0600).
