...
Tlon is a decentralized messenger built on Urbit. NEMO connects to your Urbit ship and can
...
```bash
nemo plugins install @nemo/tlon
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/tlon
```
...
Use these with `nemo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, NEMO replies in-thread.
