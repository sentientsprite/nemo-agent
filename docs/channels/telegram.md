...
```
    ...
    - or read `chat.id` from `nemo logs --follow`
...
- DM messages can carry `message_thread_id`; NEMO routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    NEMO can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, NEMO retries as plain text.
...
```
    ...
    - `~/.nemo/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, NEMO enqueues system events like:
...
```bash
nemo message send --channel telegram --target 123456789 --message "hi"
nemo message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `nemo channels status` warns when config expects unmentioned group messages.
    - `nemo channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `nemo logs --follow` for skip reasons
