...
```bash
nemo plugins install @nemo/msteams
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
NEMO will offer the local install path automatically.
...
3. Configure NEMO with those credentials.
...
- Talk to NEMO via Teams DMs, group chats, or channels.
...
- On startup, NEMO resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.nemo/nemo.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring NEMO, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `nemo-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `nemo plugins install @nemo/msteams`
   - From a local checkout: `nemo plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "NEMO in Teams", "full": "NEMO in Teams" },
...
NEMO handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, NEMO only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/NEMOShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

NEMO sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `nemo message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.nemo/msteams-polls.json`.
...
```bash
nemo message send --channel msteams \
...
nemo message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
nemo message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
nemo message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
nemo message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
