...
Matrix is an open, decentralized messaging protocol. NEMO connects as a Matrix **user**
...
```bash
nemo plugins install @nemo/matrix
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `nemo plugins install @nemo/matrix`
   - From a local checkout: `nemo plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: NEMO calls the same
     login endpoint, stores the access token in `~/.nemo/credentials/matrix/credentials.json`,
...
- On first connection, NEMO requests device verification from your other sessions.
...
`~/.nemo/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `nemo pairing list matrix`
  - `nemo pairing approve matrix <CODE>`
...
- On startup, NEMO resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
