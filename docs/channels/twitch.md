...
read_when:
  - Setting up Twitch chat integration for NEMO
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. NEMO connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
nemo plugins install @nemo/twitch
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `NEMO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nemo", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NEMO_TWITCH_ACCESS_TOKEN env var)
...
```bash
NEMO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
