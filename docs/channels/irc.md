---
title: IRC
description: Connect NEMO to IRC channels and direct messages.
---

Use IRC when you want NEMO in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.nemo/nemo.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, NEMO defaults to **mention-gating** in group contexts.
