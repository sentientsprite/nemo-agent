...
read_when:
  - You want NEMO to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables NEMO to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`nemo onboard`) and `nemo channels add` list optional channel plugins.
...
```bash
nemo plugins install @nemo/nostr
```

Use a local checkout (dev workflows):

```bash
nemo plugins install --link <path-to-nemo>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "nemo@example.com",
        "lud16": "nemo@example.com"
      }
    }
  }
}
