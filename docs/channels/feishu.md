...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects NEMO to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
nemo plugins install @nemo/feishu
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/feishu
```
...
If you just installed NEMO, run the wizard:
...
2. Configuring app credentials in NEMO
...
1. You already ran `nemo channels add` for Feishu
2. The gateway is running (`nemo gateway status`)
...
```bash
nemo pairing approve feishu <CODE>
```
...
2. Run `nemo logs --follow` and look for `chat_id`
...
2. Run `nemo logs --follow` and look for `open_id`
...
| `nemo gateway restart` | Restart gateway service       |
| `nemo logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `nemo gateway status`
6. Check logs: `nemo logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "nemod-fan",
        workspace: "/home/user/nemod-fan",
        agentDir: "/home/user/.nemo/agents/nemod-fan/agent",
      },
      {
        id: "nemod-xi",
        workspace: "/home/user/nemod-xi",
        agentDir: "/home/user/.nemo/agents/nemod-xi/agent",
      },
    ],
  },
  bindings: [
