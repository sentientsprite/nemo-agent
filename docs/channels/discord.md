...
```bash
nemo pairing list discord
nemo pairing approve discord <CODE>
```
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in NEMO config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces NEMO auth and returns "not authorized".
...
- If command deploy/state is stale, restart gateway and re-check with `nemo channels status --probe`.
