...
```bash
nemo plugins install @nemo/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
nemo plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "NEMO" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `nemo pairing list nextcloud-talk`
  - `nemo pairing approve nextcloud-talk <CODE>`
