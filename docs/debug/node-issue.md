...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in NEMO
...
Running NEMO via Node with `tsx` fails at startup with:

```
[nemo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `nemo status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch nemo.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node nemo.mjs status` works on Node 25.
