...
- `/usage cost` in chats: local cost summary aggregated from NEMO session logs.
- CLI: `nemo status --usage` prints a full per-provider breakdown.
- CLI: `nemo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
