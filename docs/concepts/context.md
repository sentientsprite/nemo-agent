...
read_when:
  - You want to understand what “context” means in NEMO
...
“Context” is **everything NEMO sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (NEMO-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How NEMO builds the system prompt

The system prompt is **NEMO-owned** and rebuilt each run. It includes:
...
By default, NEMO injects a fixed set of workspace files (if present):
