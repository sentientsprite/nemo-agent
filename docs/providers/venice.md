---
summary: "Use Venice AI privacy-focused models in NEMO"
read_when:
  - You want privacy-focused inference in NEMO
...
```bash
nemo onboard --auth-choice venice-api-key
```
...
```bash
nemo chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, NEMO shows all available Venice models. Pick based on your needs:
...
```bash
nemo models set venice/claude-opus-45
nemo models set venice/llama-3.3-70b
```

List all available models:

```bash
nemo models list | grep venice
```

## Configure via `nemo configure`

1. Run `nemo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
NEMO automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
nemo chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
nemo chat --model venice/claude-opus-45

# Use uncensored model
nemo chat --model venice/venice-uncensored

# Use vision model with image
nemo chat --model venice/qwen3-vl-235b-a22b

# Use coding model
nemo chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `nemo models list` to see currently available models. Some models may be temporarily offline.
