---
summary: "Use MiniMax M2.1 in NEMO"
read_when:
  - You want MiniMax models in NEMO
...
```bash
nemo plugins enable minimax-portal-auth  # skip if already loaded.
nemo gateway restart  # restart if gateway is already running
nemo onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/nemo/nemo/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `nemo.json`:
...
- Use `nemo models list` and `nemo models set minimax/MiniMax-M2.1` to switch.
...
- Running `nemo configure` and selecting **MiniMax M2.1**, or
