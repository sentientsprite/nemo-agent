...
所有日志配置都在 `~/.nemo/nemo.json` 的 `logging` 下。
...
- NEMO 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `nemo plugins enable diagnostics-otel` 启用插件。
...
- `nemo.tokens`（计数器，属性：`nemo.token`、`nemo.channel`、`nemo.provider`、`nemo.model`）
- `nemo.cost.usd`（计数器，属性：`nemo.channel`、`nemo.provider`、`nemo.model`）
- `nemo.run.duration_ms`（直方图，属性：`nemo.channel`、`nemo.provider`、`nemo.model`）
- `nemo.context.tokens`（直方图，属性：`nemo.context`、`nemo.channel`、`nemo.provider`、`nemo.model`）

消息流：

- `nemo.webhook.received`（计数器，属性：`nemo.channel`、`nemo.webhook`）
- `nemo.webhook.error`（计数器，属性：`nemo.channel`、`nemo.webhook`）
- `nemo.webhook.duration_ms`（直方图，属性：`nemo.channel`、`nemo.webhook`）
- `nemo.message.queued`（计数器，属性：`nemo.channel`、`nemo.source`）
- `nemo.message.processed`（计数器，属性：`nemo.channel`、`nemo.outcome`）
- `nemo.message.duration_ms`（直方图，属性：`nemo.channel`、`nemo.outcome`）

队列 + 会话：

- `nemo.queue.lane.enqueue`（计数器，属性：`nemo.lane`）
- `nemo.queue.lane.dequeue`（计数器，属性：`nemo.lane`）
- `nemo.queue.depth`（直方图，属性：`nemo.lane` 或 `nemo.channel=heartbeat`）
- `nemo.queue.wait_ms`（直方图，属性：`nemo.lane`）
- `nemo.session.state`（计数器，属性：`nemo.state`、`nemo.reason`）
- `nemo.session.stuck`（计数器，属性：`nemo.state`）
- `nemo.session.stuck_age_ms`（直方图，属性：`nemo.state`）
- `nemo.run.attempt`（计数器，属性：`nemo.attempt`）
...
- `nemo.webhook.error`
  - `nemo.channel`、`nemo.webhook`、`nemo.chatId`、`nemo.error`
- `nemo.message.processed`
  - `nemo.channel`、`nemo.outcome`、`nemo.chatId`、`nemo.messageId`、`nemo.sessionKey`、`nemo.sessionId`、`nemo.reason`
- `nemo.session.stuck`
  - `nemo.state`、`nemo.ageMs`、`nemo.queueDepth`、`nemo.sessionKey`、`nemo.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `nemo doctor`。
