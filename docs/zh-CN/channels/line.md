...
LINE 通过 LINE Messaging API 连接到 NEMO。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
nemo plugins install @nemo/line
```

本地检出（从 git 仓库运行时）：

```bash
nemo plugins install ./extensions/line
```
...
```bash
nemo pairing list line
nemo pairing approve line <CODE>
```
