...
# Check auth status
termux-toast "Checking NEMO auth..."

STATUS=$(ssh "$SERVER" '$HOME/nemo/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/nemo/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|NEMO_EXPIRING|NEMO_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/nemo/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|NEMO_EXPIRED|NEMO_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: NEMO won't work until re-auth"
                ;;
        esac
        ;;
