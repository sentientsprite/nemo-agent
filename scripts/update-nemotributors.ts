...
import type { ApiContributor, Entry, MapConfig, User } from "./update-nemotributors.types.js";

const REPO = "nemo/nemo";
const PER_LINE = 10;

const mapPath = resolve("scripts/nemotributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing nemotributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README nemotributors: ${entries.length} entries`);
