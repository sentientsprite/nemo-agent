...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/nemo/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "NEMO Re-Auth" -c "After getting key, run: ssh $SERVER '~/nemo/scripts/mobile-reauth.sh'" --id nemo-auth
        ;;
    *)
