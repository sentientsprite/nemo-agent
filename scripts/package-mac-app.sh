...
# Build and bundle NEMO into a minimal .app we can open.
# Outputs to dist/NEMO.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/NEMO.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="NEMO"
BUNDLE_ID="${BUNDLE_ID:-ai.nemo.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/nemo/nemo/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/NEMO/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :NEMOBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :NEMOGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/NEMO"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/NEMO"
fi
chmod +x "$APP_ROOT/Contents/MacOS/NEMO"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/NEMO" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/NEMO/Resources/NEMO.icns" "$APP_ROOT/Contents/Resources/NEMO.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/NEMO/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying NEMOKit resources"
NEMOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/NEMOKit_NEMOKit.bundle"
if [ -d "$NEMOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/NEMOKit_NEMOKit.bundle"
  cp -R "$NEMOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/NEMOKit_NEMOKit.bundle"
else
  echo "WARN: NEMOKit resource bundle not found at $NEMOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running NEMO"
