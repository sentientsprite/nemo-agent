#!/usr/bin/env bash
# Reset NEMO like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${NEMO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="NEMO.app/Contents/MacOS/NEMO"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/NEMO"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/NEMO"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/NEMO"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.nemo.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/nemo-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${NEMO_RESTART_LOG:-/tmp/nemo-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${NEMO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.nemo/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  NEMO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node nemo.mjs daemon install --force --runtime node"
      log "  node nemo.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.nemo/disable-launchagent"
      log ""
...
kill_all_nemo() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "NEMO" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.nemo.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing NEMO instances"
kill_all_nemo
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product NEMO"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/NEMO.app" ]]; then
    APP_BUNDLE="/Applications/NEMO.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/NEMO.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/NEMO.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/NEMO.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set NEMO_APP_BUNDLE to your installed NEMO.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nemo.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nemo.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".nemo", "nemo.json"), "utf8");
