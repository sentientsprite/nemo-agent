...
IMAGE_NAME="nemo-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="nemo-net-e2e-$$"
GW_NAME="nemo-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "NEMO_GATEWAY_TOKEN=$TOKEN" \
	  -e "NEMO_SKIP_CHANNELS=1" \
	  -e "NEMO_SKIP_GMAIL_WATCHER=1" \
	  -e "NEMO_SKIP_CRON=1" \
	  -e "NEMO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
