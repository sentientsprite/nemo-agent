...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    NEMO_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    NEMO_ENTRY="dist/index.js"
	  else
	  ...
	  home_dir=$(mktemp -d "/tmp/nemo-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.nemo/extensions/demo-plugin"

  cat > "$HOME/.nemo/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.nemo/extensions/demo-plugin/nemo.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NEMO_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/nemo-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nemo/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/nemo.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$NEMO_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$NEMO_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/nemo-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nemo/demo-plugin-dir",
  "version": "0.0.1",
  "nemo": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/nemo.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NEMO_ENTRY" plugins install "$dir_plugin"
	  node "$NEMO_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/nemo-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@nemo/demo-plugin-file",
  "version": "0.0.1",
  "nemo": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/nemo.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$NEMO_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$NEMO_ENTRY" plugins list --json > /tmp/plugins4.json
