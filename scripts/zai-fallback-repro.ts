...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "nemo.json");
  ...
  const sessionId =
    process.env.NEMO_ZAI_FALLBACK_SESSION_ID ??
    process.env.NEMO_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["nemo", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["nemo", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
