...
BASE_IMAGE="${BASE_IMAGE:-nemo-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-nemo-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=nemo.sandbox=1)
NOTE
