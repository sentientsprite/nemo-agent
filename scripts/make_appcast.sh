...
ZIP=${1:?"Usage: $0 NEMO-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/nemo/nemo/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^NEMO-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/nemo/nemo/releases/download/v${VERSION}/"}
