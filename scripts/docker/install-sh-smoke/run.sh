...
INSTALL_URL="${NEMO_INSTALL_URL:-https://nemo.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${NEMO_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NEMO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="nemo"
PACKAGE_NAME="${NEMO_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (NEMO_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${NEMO_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${NEMO_INSTALL_LATEST_OUT:-}"
fi
