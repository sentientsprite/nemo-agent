 ...
 * `nemo update`. Use it to verify the completion installation flow
...
\`nemo update\`, \`nemo doctor\`, and \`nemo onboard\`.
...
async function main() {
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nemo completion --install\` later to enable.`));
    return;
  }
