...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "nemolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
nemolog - NEMO Logging Utility

USAGE:
    nemolog [OPTIONS]

DESCRIPTION:
    View NEMO logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    NEMO logs flow through the macOS unified log (subsystem: ai.nemo).
...
QUICK START:
    nemolog -n 100             Show last 100 lines from all components
    nemolog -f                 Follow logs in real-time
    nemolog -e                 Show only errors
    nemolog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    nemolog                   Show last 50 lines from past 5 minutes (default)
    nemolog -f                Stream logs continuously
    nemolog -n 100            Show last 100 lines
    nemolog -e                Show only recent errors
    nemolog -l 30m -n 200     Show last 200 lines from past 30 minutes
    nemolog -c ServerManager  Show recent ServerManager logs
    nemolog -s "fail"         Search for "fail" in recent logs
    nemolog --server -e       Show recent server errors
    nemolog -f -d             Stream debug logs continuously
