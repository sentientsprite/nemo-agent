...
#
# Suggested cron: */30 * * * * /home/admin/nemo/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send NEMO notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., nemo-alerts)
...
STATE_FILE="$HOME/.nemo/auth-monitor-state"
...
send_notification() {
    ...
    # Send via NEMO if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use nemo
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via NEMO to $NOTIFY_PHONE..."
            nemo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! NEMO is down. Run: ssh l36 '~/nemo/scripts/mobile-reauth.sh'" "urgent"
    exit 1
