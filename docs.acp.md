...
This document describes how the NEMO ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`nemo acp` exposes an ACP agent over stdio and forwards prompts to a running
NEMO Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a NEMO Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `nemo acp` over stdio.

Example config:

```bash
nemo config set gateway.remote.url wss://gateway-host:18789
nemo config set gateway.remote.token <token>
```

Example run:

```bash
nemo acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
nemo acp --session agent:main:main
nemo acp --session agent:design:main
nemo acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “NEMO ACP” to start a thread.

## Execution Model

- ACP client spawns `nemo acp` and speaks ACP messages over stdio.
...
```bash
nemo acp --session agent:main:main
nemo acp --session-label "support inbox"
nemo acp --reset-session
```
...
`nemo acp` resolves the Gateway URL and auth from CLI flags or config:
