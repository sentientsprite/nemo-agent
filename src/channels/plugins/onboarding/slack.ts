...
function setSlackDmPolicy(cfg: NEMOConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "NEMO";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for NEMO`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: NEMOConfig, allowFrom: string[]): NEMOConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
