...
${formatHelpExamples([
  ['nemo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['nemo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'nemo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'nemo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['nemo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.nemo.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.nemo.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['nemo agents set-identity --agent main --name "NEMO" --emoji "🦞"', "Set name + emoji."],
  ["nemo agents set-identity --agent main --avatar avatars/nemo.png", "Set avatar path."],
  [
    "nemo agents set-identity --workspace ~/.nemo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "nemo agents set-identity --identity-file ~/.nemo/workspace/IDENTITY.md --agent main",
