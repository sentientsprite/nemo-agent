...
import { resolveNEMOPackageRoot } from "../infra/nemo-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "nemo doctor entry": "Checking doctor entrypoint",
  "nemo doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "The lobster has molted. Harder shell, sharper nemos.",
...
const DEFAULT_PACKAGE_NAME = "nemo";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const NEMO_REPO_URL = "https://github.com/nemo/nemo.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("nemo@")) {
    return trimmed.slice("nemo@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "nemo.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("nemo completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.NEMO_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", NEMO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `NEMO_GIT_DIR points at a non-git directory: ${params.dir}. Set NEMO_GIT_DIR to an empty folder or an nemo checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`NEMO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("NEMO update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating NEMO..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this NEMO install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("nemo doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("nemo gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g nemo@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g nemo@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.NEMO_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.NEMO_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("nemo gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nemo doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("nemo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nemo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `nemo update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `NEMO_GIT_DIR points at a non-git directory: ${gitDir}. Set NEMO_GIT_DIR to an empty folder or an nemo checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via NEMO_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update NEMO to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["nemo update", "Update a source checkout (git)"],
        ["nemo update --channel beta", "Switch to beta channel (git + npm)"],
        ["nemo update --channel dev", "Switch to dev channel (git + npm)"],
        ["nemo update --tag beta", "One-off update to a dist-tag or version"],
        ["nemo update --no-restart", "Update without restarting the service"],
        ["nemo update --json", "Output result as JSON"],
        ["nemo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["nemo update wizard", "Interactive update wizard"],
        ["nemo --update", "Shorthand for nemo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run nemo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.nemo.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nemo update status", "Show channel + version status."],
          ["nemo update status --json", "JSON output."],
          ["nemo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.nemo.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
