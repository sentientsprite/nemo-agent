...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      NEMO_SKIP_GMAIL_WATCHER: "1",
      NEMO_SKIP_CRON: "1",
      NEMO_SKIP_BROWSER_CONTROL_SERVER: "1",
      NEMO_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "nemo-entry-bootstrap.mjs");
