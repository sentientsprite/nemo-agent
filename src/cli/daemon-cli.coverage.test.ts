...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    NEMO_STATE_DIR: process.env.NEMO_STATE_DIR,
    NEMO_CONFIG_PATH: process.env.NEMO_CONFIG_PATH,
    NEMO_GATEWAY_PORT: process.env.NEMO_GATEWAY_PORT,
    NEMO_PROFILE: process.env.NEMO_PROFILE,
  };

  beforeEach(() => {
    process.env.NEMO_STATE_DIR = "/tmp/nemo-cli-state";
    process.env.NEMO_CONFIG_PATH = "/tmp/nemo-cli-state/nemo.json";
    delete process.env.NEMO_GATEWAY_PORT;
    delete process.env.NEMO_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.NEMO_STATE_DIR !== undefined) {
      process.env.NEMO_STATE_DIR = originalEnv.NEMO_STATE_DIR;
    } else {
      delete process.env.NEMO_STATE_DIR;
    }

    if (originalEnv.NEMO_CONFIG_PATH !== undefined) {
      process.env.NEMO_CONFIG_PATH = originalEnv.NEMO_CONFIG_PATH;
    } else {
      delete process.env.NEMO_CONFIG_PATH;
    }

    if (originalEnv.NEMO_GATEWAY_PORT !== undefined) {
      process.env.NEMO_GATEWAY_PORT = originalEnv.NEMO_GATEWAY_PORT;
    } else {
      delete process.env.NEMO_GATEWAY_PORT;
    }

    if (originalEnv.NEMO_PROFILE !== undefined) {
      process.env.NEMO_PROFILE = originalEnv.NEMO_PROFILE;
    } else {
      delete process.env.NEMO_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        NEMO_PROFILE: "dev",
        NEMO_STATE_DIR: "/tmp/nemo-daemon-state",
        NEMO_CONFIG_PATH: "/tmp/nemo-daemon-state/nemo.json",
        NEMO_GATEWAY_PORT: "19001",
      },
