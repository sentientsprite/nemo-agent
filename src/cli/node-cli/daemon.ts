...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("nemo node install"), formatCliCommand("nemo node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("nemo node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    NEMO_LOG_PREFIX: baseEnv.NEMO_LOG_PREFIX ?? "node",
