...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# NEMO Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(nemo completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(nemo completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "nemo") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c nemo -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'nemo gateway' and not 'nemo other gateway', we need to check the sequence.
