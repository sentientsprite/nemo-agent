...
const SANDBOX_EXAMPLES = {
  main: [
    ["nemo sandbox list", "List all sandbox containers."],
    ["nemo sandbox list --browser", "List only browser containers."],
    ["nemo sandbox recreate --all", "Recreate all containers."],
    ["nemo sandbox recreate --session main", "Recreate a specific session."],
    ["nemo sandbox recreate --agent mybot", "Recreate agent containers."],
    ["nemo sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["nemo sandbox list", "List all sandbox containers."],
    ["nemo sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["nemo sandbox recreate --all", "Recreate all containers."],
    ["nemo sandbox recreate --session main", "Recreate a specific session."],
    ["nemo sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["nemo sandbox recreate --browser --all", "Recreate only browser containers."],
    ["nemo sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["nemo sandbox explain", "Show effective sandbox config."],
    ["nemo sandbox explain --session agent:main:main", "Explain a specific session."],
    ["nemo sandbox explain --agent work", "Explain an agent sandbox."],
    ["nemo sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.nemo.ai/cli/sandbox")}\n`,
    )
    .action(() => {
