...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "nemo" | "nemo-agent" | "nemo-agent";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["nemo", "nemo-agent", "nemo-agent"] as const;
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.NEMO_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["nemo_service_marker"];
  const kindKeys = ["nemo_service_kind"];
...
function isNEMOGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.nemo.");
}

function isNEMOGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("nemo-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isNEMOGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("nemo gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("nemo-agent") || lower.includes("nemo-agent");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "nemo-agent" : "nemo-agent",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "nemo" && isNEMOGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "nemo" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "nemo" && isNEMOGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
