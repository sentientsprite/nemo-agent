...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NEMO_GATEWAY_PORT).toBe("18789");
    expect(env.NEMO_GATEWAY_TOKEN).toBe("secret");
    expect(env.NEMO_SERVICE_MARKER).toBe("nemo");
    expect(env.NEMO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.NEMO_SERVICE_VERSION).toBe("string");
    expect(env.NEMO_SYSTEMD_UNIT).toBe("nemo-gateway.service");
    if (process.platform === "darwin") {
      expect(env.NEMO_LAUNCHD_LABEL).toBe("ai.nemo.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NEMO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NEMO_SYSTEMD_UNIT).toBe("nemo-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.NEMO_LAUNCHD_LABEL).toBe("ai.nemo.work");
    }
  });
});
