...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real nemo.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.NEMO_SESSION_CACHE_TTL_MS;
    process.env.NEMO_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.NEMO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.NEMO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
