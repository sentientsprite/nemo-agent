...
import type { NEMOConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NEMOConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: NEMOConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NEMOSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NEMOConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NEMOConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as NEMOConfig)),
    ),
  };
}
