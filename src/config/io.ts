...
import type { NEMOConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareNEMOVersions } from "./version.js";
...
function coerceConfig(value: unknown): NEMOConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as NEMOConfig;
}
...
function stampConfigVersion(cfg: NEMOConfig): NEMOConfig {
...
function warnIfConfigFromFuture(cfg: NEMOConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareNEMOVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer NEMO (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: NEMOConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): NEMOConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as NEMOConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as NEMOConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: NEMOConfig) {
    clearConfigCache();
...
// module scope. `NEMO_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NEMO_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NEMO_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): NEMOConfig {
...
export async function writeConfigFile(cfg: NEMOConfig): Promise<void> {
  clearConfigCache();
