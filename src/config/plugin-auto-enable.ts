import type { NEMOConfig } from "./config.js";
...
function resolveChannelConfig(cfg: NEMOConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: NEMOConfig): boolean {
...
function isIMessageConfigured(cfg: NEMOConfig): boolean {
...
function isWhatsAppConfigured(cfg: NEMOConfig): boolean {
...
function isGenericChannelConfigured(cfg: NEMOConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: NEMOConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: NEMOConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: NEMOConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: NEMOConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: NEMOConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: NEMOConfig, pluginId: string): NEMOConfig {
...
function registerPluginEntry(cfg: NEMOConfig, pluginId: string): NEMOConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
