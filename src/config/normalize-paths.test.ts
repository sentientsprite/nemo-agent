  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.nemo/logs/nemo.log" },
        hooks: {
          path: "~/.nemo/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".nemo", "logs", "nemo.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".nemo", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".nemo", "telegram.token"),
      );
