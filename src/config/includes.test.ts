...
const ETC_NEMO_DIR = path.join(ROOT_DIR, "etc", "nemo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "nemo.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcNEMOPath(...parts: string[]) {
  return path.join(ETC_NEMO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcNEMOPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "nemo.json"))).toEqual({
      shared: true,
    });
  });
});
