...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nemo",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nemo#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-sessions-"));
  ...
  it("derives session transcripts dir from NEMO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NEMO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.NEMO_STATE_DIR = "/custom/state";
    try {
