...
export function parseNEMOVersion(raw: string | null | undefined): NEMOVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareNEMOVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseNEMOVersion(a);
  const parsedB = parseNEMOVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
