...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "NEMO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.NEMO_STATE_DIR = path.join(home, ".nemo");
    ...
    await fs.mkdir(path.join(home, ".nemo"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.nemo${path.sep}media`);
