...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.NEMO_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.NEMO_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.NEMO_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[nemo] ${parsed.error}`);
    process.exit(2);
  }
