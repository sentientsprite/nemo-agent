...
import { scheduleGatewaySigusr1Restart, triggerNEMORestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting NEMO in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerNEMORestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting NEMO via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
