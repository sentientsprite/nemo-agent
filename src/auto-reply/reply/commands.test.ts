...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-commands-"));
...
function buildParams(commandBody: string, cfg: NEMOConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
