...
import type { NEMOConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: NEMOConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: NEMOConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as NEMOConfig),
...
export function getCommandDetection(_cfg?: NEMOConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: NEMOConfig) {
