...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NEMO_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("NEMO Canvas");
    } finally {
      await handler.close();
