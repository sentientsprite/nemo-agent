...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("nemoCanvasA2UIAction");
    expect(out).toContain("nemoSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("nemoSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.nemoA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__nemo__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("nemo-a2ui-host");
      expect(html).toContain("nemoCanvasA2UIAction");

      const bundleRes = await fetch(`http://127.0.0.1:${server.port}/__nemo__/a2ui/a2ui.bundle.js`);
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("nemoA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
