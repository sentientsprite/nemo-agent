...
vi.mock("../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/nemo-sessions.json"),
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      ...
      const cfg = {
        ...
        session: { store: "/tmp/nemo-sessions.json" },
        channels: {
          discord: {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bnemo\\b"] },
        },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "NEMO" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/nemo" },
      session: { store: "/tmp/nemo-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
