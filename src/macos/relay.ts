...
async function main() {
  ...
  const { ensureNEMOCliOnPath } = await import("../infra/path-env.js");
  ensureNEMOCliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[nemo] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[nemo] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
