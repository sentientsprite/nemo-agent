...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nemo-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.nemo when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nemo-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers NEMO_HOME over HOME", () => {
    vi.stubEnv("NEMO_HOME", "/srv/nemo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/nemo-home"));
...
describe("shortenHomePath", () => {
  it("uses $NEMO_HOME prefix when NEMO_HOME is set", () => {
    vi.stubEnv("NEMO_HOME", "/srv/nemo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/nemo-home")}/.nemo/nemo.json`)).toBe(
      "$NEMO_HOME/.nemo/nemo.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $NEMO_HOME replacement when NEMO_HOME is set", () => {
    vi.stubEnv("NEMO_HOME", "/srv/nemo-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/nemo")).toBe(path.resolve(os.homedir(), "nemo"));
  });
  ...
  it("prefers NEMO_HOME for tilde expansion", () => {
    vi.stubEnv("NEMO_HOME", "/srv/nemo-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/nemo")).toBe(path.resolve("/srv/nemo-home", "nemo"));
