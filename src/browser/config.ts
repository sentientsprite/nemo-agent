import type { BrowserConfig, BrowserProfileConfig, NEMOConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "nemo" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_NEMO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "nemo" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NEMO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NEMO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an NEMO browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for nemo/nemo CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NEMOConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NEMO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "nemo";
