...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const nemo = resolveProfile(resolved, "nemo");
    expect(nemo?.driver).toBe("nemo");
    expect(nemo?.cdpPort).toBe(18800);
    expect(nemo?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from NEMO_GATEWAY_PORT when unset", () => {
    const prev = process.env.NEMO_GATEWAY_PORT;
    process.env.NEMO_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const nemo = resolveProfile(resolved, "nemo");
      expect(nemo?.cdpPort).toBe(19012);
      expect(nemo?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.NEMO_GATEWAY_PORT;
      } else {
        process.env.NEMO_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const nemo = resolveProfile(resolved, "nemo");
      expect(nemo?.cdpPort).toBe(19022);
      expect(nemo?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.NEMO_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "nemo");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        nemo: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("nemo");
  });
});
