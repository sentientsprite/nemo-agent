...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the NEMO gateway (NEMO.app menubar, or \`${formatCliCommand("nemo gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the NEMO browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the NEMO browser control service. ${hint} (${msg})`);
}
