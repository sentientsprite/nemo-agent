...
import { loadNEMOPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.NEMO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `nemo-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "nemo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.NEMO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadNEMOPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.NEMO_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadNEMOPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadNEMOPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
