...
import { discoverNEMOPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NEMOPluginDefinition;
  register?: NEMOPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as NEMOPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as NEMOPluginDefinition;
...
export function loadNEMOPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverNEMOPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "nemo/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: NEMOPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as NEMOPluginModule;
    } catch (err) {
