...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `nemo-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.nemo/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without nemo.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@nemo/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("nemo.extensions");
  });
