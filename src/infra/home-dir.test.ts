...
describe("resolveEffectiveHomeDir", () => {
  it("prefers NEMO_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/nemo-home"));
  });
  ...
  it("expands NEMO_HOME when set to ~", () => {
    const env = {
      NEMO_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for NEMO_HOME", () => {
    const result = resolveRequiredHomeDir(
      { NEMO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when NEMO_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ NEMO_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { NEMO_HOME: "/srv/nemo-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/nemo-home")}/x`);
  });
