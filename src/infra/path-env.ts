...
function candidateBinDirs(opts: EnsureNEMOPathOpts): string[] {
  ...
  // Bundled macOS app: `nemo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nemo");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/nemo` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "nemo"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `nemo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNEMOCliOnPath(opts: EnsureNEMOPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NEMO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.NEMO_PATH_BOOTSTRAPPED = "1";
