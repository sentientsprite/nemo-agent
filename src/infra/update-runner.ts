...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "nemo.mjs");
    ...
    const doctorStep = await runStep(
      step("nemo doctor", doctorArgv, gitRoot, { NEMO_UPDATE_IN_PROGRESS: "1" }),
    );
