...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: NEMOConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: NEMOConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: NEMOConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: NEMOConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: NEMOConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: NEMOConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: NEMOConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function resolveHeartbeatSession(cfg: NEMOConfig, agentId?: string, heartbeat?: HeartbeatConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: NEMOConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
