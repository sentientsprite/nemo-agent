...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`$ORIGIN nemo.internal.`);
    ...
    expect(txt).toContain(`_nemo-gw._tcp IN PTR studio-london._nemo-gw._tcp`);
    expect(txt).toContain(`studio-london._nemo-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (NEMO)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/nemo`);
  });
