...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NEMO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nemo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nemo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nemo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nemo-gw");
  ...
  records.push(`_nemo-gw._tcp IN PTR ${instanceLabel}._nemo-gw._tcp`);
  records.push(`${instanceLabel}._nemo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nemo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nemo-content-hash: ${contentHash}\n${contentBody}`;
}
