...
import { resolveNEMOPackageRoot, resolveNEMOPackageRootSync } from "./nemo-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveNEMOPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "nemo" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveNEMOPackageRootSync({
    argv1,
