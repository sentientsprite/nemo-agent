...
import { ensureNEMOCliOnPath } from "./path-env.js";

describe("ensureNEMOCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling nemo exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "nemo");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.NEMO_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.NEMO_PATH_BOOTSTRAPPED;
      try {
        ensureNEMOCliOnPath({
          execPath: cliPath,
          cwd: tmp,
        ...
        if (originalFlag === undefined) {
          ...
          process.env.NEMO_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "nemo");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "nemo");
