...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "nemo-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    NEMO_GATEWAY_TOKEN: "test-token",
    NEMO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    NEMO_WORKSPACE_DIR: join(sandbox.rootDir, "nemo"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      NEMO_DOCKER_APT_PACKAGES: undefined,
      NEMO_EXTRA_MOUNTS: undefined,
      NEMO_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("NEMO_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("NEMO_EXTRA_MOUNTS=");
    expect(envFile).toContain("NEMO_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    expect(extraCompose).toContain("nemo-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("nemo-home:");
  });
  ...
  it("plumbs NEMO_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      NEMO_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      NEMO_EXTRA_MOUNTS: "",
      NEMO_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NEMO_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg NEMO_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
