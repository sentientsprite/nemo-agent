...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as NEMOConfig);
...
export function listChannelAgentTools(params: { cfg?: NEMOConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: NEMOConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
