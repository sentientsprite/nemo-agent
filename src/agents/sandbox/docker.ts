...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "nemo.sandbox=1");
  args.push("--label", `nemo.sessionKey=${params.scopeKey}`);
  args.push("--label", `nemo.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `nemo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("nemo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`nemo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`nemo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("nemo sandbox recreate --all");
}
