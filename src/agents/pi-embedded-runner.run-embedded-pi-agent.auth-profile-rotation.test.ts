...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): NEMOConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-workspace-"));
    try {
