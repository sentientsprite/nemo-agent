...
import { __testing, createNEMOCodingTools } from "./pi-tools.js";
...
describe("createNEMOCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNEMOCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "nemo-outside.txt");
