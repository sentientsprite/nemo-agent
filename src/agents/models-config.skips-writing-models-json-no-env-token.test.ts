...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "nemo-models-" });
}

const MODELS_CONFIG: NEMOConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureNEMOModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureNEMOModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureNEMOModelsJson } = await import("./models-config.js");
      const { resolveNEMOAgentDir } = await import("./agent-paths.js");

      await ensureNEMOModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveNEMOAgentDir(), "models.json");
