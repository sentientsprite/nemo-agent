...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.NEMO_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.NEMO_BUNDLED_SKILLS_DIR;
    } else {
      process.env.NEMO_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "nemo" }));
