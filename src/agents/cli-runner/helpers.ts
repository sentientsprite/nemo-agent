...
/**
 * Cleanup suspended NEMO CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: NEMOConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-cli-images-"));
  const paths: string[] = [];
