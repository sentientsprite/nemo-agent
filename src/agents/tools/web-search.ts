...
type WebSearchConfig = NonNullable<NEMOConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: NEMOConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("nemo configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.nemo.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://nemo.ai",
      "X-Title": "NEMO Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
