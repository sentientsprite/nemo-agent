...
const defaultTools = createNEMOCodingTools();

describe("createNEMOCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from nemo read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "nemo-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "nemo-workspace"),
    ...
    const tools = createNEMOCodingTools({ sandbox });
