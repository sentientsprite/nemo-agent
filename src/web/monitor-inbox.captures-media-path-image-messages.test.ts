...
describe("web monitor inbox", () => {
  beforeEach(() => {
    ...
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "nemo-auth-"));
  });
  ...
  it("logs inbound bodies to file", async () => {
    const logPath = path.join(os.tmpdir(), `nemo-log-test-${crypto.randomUUID()}.log`);
