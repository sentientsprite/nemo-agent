...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `nemo-hook-install-${randomUUID()}`);
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        nemo: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({
        name: "@nemo/test-hooks",
        version: "0.0.1",
        nemo: { hooks: ["./hooks/one-hook"] },
