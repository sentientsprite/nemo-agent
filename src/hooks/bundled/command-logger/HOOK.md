...
homepage: https://docs.nemo.ai/hooks#command-logger
metadata:
  {
    "nemo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.nemo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nemo/logs/commands.log ~/.nemo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nemo`:
   ```
   /home/username/.nemo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
