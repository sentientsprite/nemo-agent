...
This directory contains hooks that ship with NEMO. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.nemo.ai/hooks/soul-evil
...
Hooks can be configured in `~/.nemo/nemo.json`:
...
2. Restart gateway: `pkill -9 -f 'nemo.*gateway' && pnpm nemo gateway`
3. Enable the hook: `nemo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.nemo.ai/hooks
