...
`;
    ...
    expect(parsed.nemo.emoji).toBe("💾");
    expect(parsed.nemo.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.nemo.emoji).toBe("📝");
    expect(parsed.nemo.events).toEqual(["command"]);
    expect(parsed.nemo.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.nemo.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"nemo": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"nemo": {"events": ["test"]}}');
  });
...
describe("resolveNEMOMetadata", () => {
  it("extracts nemo metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveNEMOMetadata(frontmatter);
  ...
  it("returns undefined when nemo key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        nemo: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with NEMO" },
            { id: "npm", kind: "npm", package: "@nemo/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@nemo/hook");
  });
...
`;
    ...
    const nemo = resolveNEMOMetadata(frontmatter);
    expect(nemo).toBeDefined();
    expect(nemo?.emoji).toBe("💾");
    expect(nemo?.events).toEqual(["command:new"]);
    expect(nemo?.requires?.config).toEqual(["workspace.dir"]);
    expect(nemo?.install?.[0].kind).toBe("bundled");
  });
