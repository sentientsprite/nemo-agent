...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.NEMO_STATE_DIR;
    prevBundledDir = process.env.NEMO_BUNDLED_HOOKS_DIR;
    process.env.NEMO_STATE_DIR = path.join(baseDir, "state");
    process.env.NEMO_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.NEMO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.NEMO_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"nemo":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
