...
function summarizeGroupPolicy(cfg: NEMOConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: NEMOConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: NEMOConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: NEMOConfig): boolean {
...
function isBrowserEnabled(cfg: NEMOConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: NEMOConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: NEMOConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep NEMO_STATE_DIR on a local-only volume and re-run "${formatCliCommand("nemo security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: NEMOConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectHooksHardeningFindings(cfg: NEMOConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: NEMOConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: NEMOConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
