...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-session-fs-test-"));
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __nemo?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__nemo?.kind).toBe("compaction");
    expect(marker.__nemo?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-session-preview-test-"));
    storePath = path.join(tmpDir, "sessions.json");
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses NEMO_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/nemo-home"), ".nemo", "sessions", "sess-1.jsonl"),
    );
  });
});
