...
import { ensureNEMOCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  process.env.NEMO_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NEMO_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NEMO_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("nemo doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("nemo doctor")}" to repair, then retry.`,
    );
  }
