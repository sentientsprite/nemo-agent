...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-nemo-message-channel") ?? "");
  const accountId = getHeader(req, "x-nemo-account-id")?.trim() || undefined;
  ...
  const allTools = createNEMOTools({
