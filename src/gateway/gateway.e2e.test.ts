...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "nemo");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.nemo-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".nemo");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "nemo.json");
      ...
      try {
        ...
        process.env.NEMO_CONFIG_PATH = prev.configPath;
        process.env.NEMO_GATEWAY_TOKEN = prev.token;
        process.env.NEMO_SKIP_CHANNELS = prev.skipChannels;
        process.env.NEMO_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.NEMO_SKIP_CRON = prev.skipCron;
        process.env.NEMO_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.NEMO_STATE_DIR = prev.stateDir;
      process.env.NEMO_CONFIG_PATH = prev.configPath;
      process.env.NEMO_GATEWAY_TOKEN = prev.token;
      process.env.NEMO_SKIP_CHANNELS = prev.skipChannels;
      process.env.NEMO_SKIP_GMAIL_WATCHER = prev.skipGmail;
