...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.NEMO_STATE_DIR;
  previousConfigPath = process.env.NEMO_CONFIG_PATH;
  previousSkipBrowserControl = process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.NEMO_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.NEMO_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.NEMO_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NEMO_STATE_DIR = path.join(tempHome, ".nemo");
  delete process.env.NEMO_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.NEMO_SKIP_GMAIL_WATCHER = "1";
  process.env.NEMO_SKIP_CANVAS_HOST = "1";
  process.env.NEMO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nemo-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "nemo-test-"))
    : path.join(tempHome, ".nemo-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.NEMO_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.NEMO_CONFIG_PATH;
    } else {
      process.env.NEMO_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.NEMO_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.NEMO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEMO_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.NEMO_SKIP_CANVAS_HOST;
    } else {
      process.env.NEMO_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.NEMO_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.NEMO_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(
  ...
  const prev = process.env.NEMO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NEMO_GATEWAY_TOKEN = fallbackToken;
  }
