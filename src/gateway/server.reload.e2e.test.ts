...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.NEMO_SKIP_CHANNELS;
    prevSkipGmail = process.env.NEMO_SKIP_GMAIL_WATCHER;
    process.env.NEMO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NEMO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NEMO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NEMO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
