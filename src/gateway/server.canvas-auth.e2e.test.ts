...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.NEMO_CONFIG_PATH;
  const prevDisableCache = process.env.NEMO_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "nemo-canvas-auth-test-"));
  const configPath = path.join(dir, "nemo.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NEMO_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.NEMO_DISABLE_CONFIG_CACHE;
    } else {
      process.env.NEMO_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
