...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.NEMO_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "nemo");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.NEMO_STATE_DIR = stateDir;
    process.env.NEMO_CONFIG_PATH = path.join(stateDir, "nemo.json");
