...
const SEARCH_TOOL = "https://docs.nemo.ai/mcp.SearchNEMO";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nemo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nemo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nemo.ai/");
      runtime.log(`Search: ${formatCliCommand('nemo docs "your query"')}`);
    }
    return;
  }
