...
function stripUnknownConfigKeys(config: NEMOConfig): {
  config: NEMOConfig;
  removed: string[];
} {
  const parsed = NEMOSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: NEMOConfig) {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".nemo");
  const targetPath = path.join(targetDir, "nemo.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".nemo-agent", "nemo-agent.json"),
    path.join(home, ".nemo-agent", "nemo-agent.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("nemo doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("nemo doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "nemo doctor --fix" to remove these keys.');
    }
  }
