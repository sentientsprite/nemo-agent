...
beforeEach(() => {
  ...
  resolveNEMOPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.NEMO_STATE_DIR;
  originalUpdateInProgress = process.env.NEMO_UPDATE_IN_PROGRESS;
  process.env.NEMO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.NEMO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.NEMO_UPDATE_IN_PROGRESS;
  } else {
    process.env.NEMO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveNEMOPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadNEMOPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/nemo-root.js", () => ({
  resolveNEMOPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemo-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.NEMO_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when NEMO_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/nemo.json",
      exists: true,
      raw: "{}",
    ...
    process.env.NEMO_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
