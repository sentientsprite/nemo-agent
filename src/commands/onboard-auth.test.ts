...
const requireAgentDir = () => {
  const agentDir = process.env.NEMO_AGENT_DIR;
  if (!agentDir) {
    throw new Error("NEMO_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.NEMO_STATE_DIR;
  const previousAgentDir = process.env.NEMO_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.NEMO_AGENT_DIR;
    } else {
      process.env.NEMO_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.NEMO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under NEMO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-oauth-"));
    process.env.NEMO_STATE_DIR = tempStateDir;
    process.env.NEMO_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEMO_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to NEMO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-minimax-"));
    process.env.NEMO_STATE_DIR = tempStateDir;
    process.env.NEMO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.NEMO_AGENT_DIR;
