...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveNEMOAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("nemo models auth setup-token")}\` or \`${formatCliCommand("nemo configure")}\`.`
          : `Run \`${formatCliCommand("nemo configure")}\` or set an API key env var.`;
