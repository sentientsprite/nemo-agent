...
import { type NEMOConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function updateConfig(mutator: (cfg: NEMOConfig) => NEMOConfig): Promise<NEMOConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: NEMOConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: NEMOConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("nemo agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
