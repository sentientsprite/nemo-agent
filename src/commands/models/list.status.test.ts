...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveNEMOAgentDir: vi.fn().mockReturnValue("/tmp/nemo-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/nemo-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveNEMOAgentDir: mocks.resolveNEMOAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveNEMOAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/nemo-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/nemo-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/nemo-agent-custom");
