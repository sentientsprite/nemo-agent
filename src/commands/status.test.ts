...
beforeAll(() => {
  previousProfile = process.env.NEMO_PROFILE;
  process.env.NEMO_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.NEMO_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/nemo-root.js", () => ({
  resolveNEMOPackageRoot: vi.fn().mockResolvedValue("/tmp/nemo"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/nemo/pnpm-lock.yaml",
      markerPath: "/tmp/nemo/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("NEMO status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("nemo status --all") ||
          l.includes("nemo --profile isolated status --all") ||
          l.includes("nemo status --all") ||
          l.includes("nemo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.NEMO_GATEWAY_TOKEN = "abcd1234";
    try {
