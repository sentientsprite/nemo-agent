...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.NEMO_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.NEMO_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("NEMO_STATE_DIR", prev.stateDir);
  restoreEnvVar("NEMO_CONFIG_PATH", prev.configPath);
  restoreEnvVar("NEMO_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("NEMO_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("NEMO_SKIP_CRON", prev.skipCron);
  restoreEnvVar("NEMO_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("NEMO_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("NEMO_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("NEMO_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.NEMO_DISABLE_CONFIG_CACHE = "1";
  delete process.env.NEMO_GATEWAY_TOKEN;
  delete process.env.NEMO_GATEWAY_PASSWORD;
  delete process.env.CUSTOM_API_KEY;

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), prefix));
  const configPath = path.join(tempHome, "nemo.json");
  process.env.HOME = tempHome;
  process.env.NEMO_STATE_DIR = tempHome;
  process.env.NEMO_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("nemo-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("nemo-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("nemo-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("nemo-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("nemo-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("nemo-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("nemo-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "nemo-onboard-custom-provider-env-fallback-",
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "nemo-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("nemo-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
