...
function resolveMode(cfg: NEMOConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveNEMOPackageRoot({
  ...
  let cfg: NEMOConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("nemo configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("nemo config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("nemo setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("nemo doctor --fix")}" to apply changes.`);
  }
