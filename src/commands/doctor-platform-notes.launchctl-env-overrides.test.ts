...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "NEMO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("NEMO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv NEMO_GATEWAY_TOKEN");
    expect(message).not.toContain("NEMO_GATEWAY_PASSWORD");
  });
