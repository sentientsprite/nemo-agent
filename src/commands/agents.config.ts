...
export function listAgentEntries(cfg: NEMOConfig): AgentEntry[] {
...
function resolveAgentName(cfg: NEMOConfig, agentId: string) {
...
function resolveAgentModel(cfg: NEMOConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NEMOConfig): AgentSummary[] {
