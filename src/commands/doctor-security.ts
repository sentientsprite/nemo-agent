...
import type { NEMOConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: NEMOConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("nemo security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nemo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nemo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nemo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nemo config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
