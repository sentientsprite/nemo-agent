/**
 * Twitch channel plugin for NEMO.
 *
...
import { buildChannelConfigSchema } from "nemo/plugin-sdk";
import type {
 ...
 * for NEMO. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: NEMOConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: NEMOConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: NEMOConfig): boolean => {
