 ...
 */
function setTwitchAccount(cfg: NEMOConfig, account: Partial<TwitchAccountConfig>): NEMOConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: NEMO_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: NEMOConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var NEMO_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.NEMO_TWITCH_ACCESS_TOKEN?.trim();
