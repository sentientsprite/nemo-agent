...
const liveEnabled = HAS_OPENAI_KEY && process.env.NEMO_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
