...
function resolveNEMORoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.NEMO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set NEMO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveNEMORoot(), "dist", "extensionAPI.js");
