...
`nemo.invoke --each`) without adding new NEMO code per workflow.
...
This extension depends on NEMO internal modules (the embedded agent runner).
It is intended to ship as a **bundled** NEMO extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.nemo/extensions` as a standalone plugin directory.
