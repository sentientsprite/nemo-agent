...
NEMO extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
nemo channels login --channel zalouser
...
```bash
nemo message send --channel zalouser --target <threadId> --message "Hello from NEMO!"
```
...
```bash
nemo channels login --channel zalouser              # Login via QR
nemo channels login --channel zalouser --account work
nemo channels status --probe
nemo channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
nemo directory self --channel zalouser
nemo directory peers list --channel zalouser --query "name"
nemo directory groups list --channel zalouser --query "work"
nemo directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
nemo message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
nemo message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `nemo channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
nemo channels login --channel zalouser --account work
nemo message send --channel zalouser --account work --target <id> --message "Hello"
