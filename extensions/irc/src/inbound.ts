...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as NEMOConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as NEMOConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as NEMOConfig);
