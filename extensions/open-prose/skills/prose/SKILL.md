...
metadata: { "nemo": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == NEMO `sessions_spawn`
- **File I/O** == NEMO `read`/`write`
- **Remote fetch** == NEMO `web_fetch` (or `exec` with curl when POST is required)
