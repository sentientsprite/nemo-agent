...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["nemoPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["nemo", "pollId"]) ??
    readNestedString(value, ["nemo", "poll", "id"]) ??
    readNestedString(value, ["data", "nemoPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "nemo", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          ...
          value: { nemoPollId: pollId, pollId },
        },
      },
    ],
  };
