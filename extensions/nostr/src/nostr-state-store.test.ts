import type { PluginRuntime } from "nemo/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NEMO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nemo-nostr-"));
  process.env.NEMO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NEMO_STATE_DIR = previous;
    }
