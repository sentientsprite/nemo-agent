import type { NEMOPluginApi, NEMOPluginService } from "nemo/plugin-sdk";
...
function normalizeDenyList(cfg: NEMOPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: NEMOPluginApi["config"]): string[] {
...
export default function register(api: NEMOPluginApi) {
  ...
  const timerService: NEMOPluginService = {
