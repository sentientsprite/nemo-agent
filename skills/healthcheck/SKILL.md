---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for NEMO deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, NEMO cron scheduling for periodic checks, or version status checks on a machine running NEMO (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running NEMO, then align it to a user-defined risk tolerance without breaking access. Use NEMO security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim NEMO changes the host firewall, SSH, or OS updates; it does not.
...
5. NEMO gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if NEMO can access sensitive data.
...
### 2) Run NEMO security audits (read-only)

As part of the default read-only checks, run `nemo security audit --deep`. Only offer alternatives if the user requests them:

1. `nemo security audit` (faster, non-probing)
2. `nemo security audit --json` (structured output)

Offer to apply NEMO safe defaults (numbered):

1. `nemo security audit --fix`

Be explicit that `--fix` only tightens NEMO defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check NEMO version/update status (read-only)

As part of the default read-only checks, run `nemo update status`.
...
- Credential hygiene notes (location of NEMO creds, prefer disk encryption)
...
After NEMO install or first hardening pass, run at least one baseline audit and version check:

- `nemo security audit`
- `nemo security audit --deep`
- `nemo update status`

Ongoing monitoring is recommended. Use the NEMO cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `nemo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `nemo update status`
...
Before creating, `nemo cron list` and match on exact `name`. If found, `nemo cron edit <id> ...`.
If not found, `nemo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `nemo update status` (preferred for source checkouts and channels)
2. `npm view nemo version` (published npm version)

## NEMO command accuracy

Use only supported commands and flags:

- `nemo security audit [--deep] [--fix] [--json]`
- `nemo status` / `nemo status --deep`
- `nemo health --json`
- `nemo update status`
- `nemo cron add|list|runs|run`

Do not invent CLI flags or imply NEMO enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by NEMO compaction:
