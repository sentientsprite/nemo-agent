...
metadata: { "nemo": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${NEMO_TMUX_SOCKET_DIR:-${NEMO_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/nemo-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nemo.sock"
SESSION=nemo-python
...
- Use `NEMO_TMUX_SOCKET_DIR` (legacy `NEMO_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$NEMO_TMUX_SOCKET_DIR/nemo.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `NEMO_TMUX_SOCKET_DIR`).
