...
import type { NEMOConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("nemo-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("nemo-bin-sherpa-");
      const modelDir = await makeTempDir("nemo-sherpa-model-");
      ...
      const cfg: NEMOConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("nemo-bin-whispercpp-");
      const modelDir = await makeTempDir("nemo-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("nemo-bin-gemini-");
      ...
      const cfg: NEMOConfig = { tools: { media: { image: {} } } };
