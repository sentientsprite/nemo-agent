...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nemo-e2e-${name}-`));
  const configDir = path.join(homeDir, ".nemo");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          NEMO_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        NEMO_GATEWAY_TOKEN: inst.gatewayToken,
        NEMO_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEMO_GATEWAY_PORT: String(gwA.port),
          NEMO_GATEWAY_TOKEN: gwA.gatewayToken,
          NEMO_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          NEMO_GATEWAY_PORT: String(gwB.port),
          NEMO_GATEWAY_TOKEN: gwB.gatewayToken,
          NEMO_GATEWAY_PASSWORD: "",
        }),
