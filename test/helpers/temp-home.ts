...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("NEMO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.NEMO_STATE_DIR = path.join(base, ".nemo");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "nemo-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".nemo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
